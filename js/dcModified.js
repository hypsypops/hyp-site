...
};dc.dateFormat = d3.time.format("%Y");
...
dc.printers.filter = function (filter) {
    ...
    if (filter) {
        if (filter instanceof Array) {
            if (filter.length >= 2)
                s = "[" + (parseInt(printSingleValue(filter[0]))+1) +" - " + printSingleValue(filter[1]) + "]";
...
dc.abstractBubbleChart = function (_chart) {
    var _maxBubbleRelativeSize = 0.5;
    var _minRadiusWithLabel = 3;

    _chart.BUBBLE_NODE_CLASS = "node";
    _chart.BUBBLE_CLASS = "bubble";
    _chart.MIN_RADIUS = 3;

    _chart = dc.multiSelectionChart(dc.colorChart(_chart));
    ...
    _chart.isSelectedNode = function (d) {
        return _chart.isFilteredBy(d.key);
    };
...
dc.pieChart = function (parent, chartGroup) {
    var DEFAULT_MIN_ANGLE_FOR_LABEL = 0.2;
...
dc.geoChoroplethChart = function (parent, chartGroup) {
    ...
    //var _geoPath = d3.svg.line()
    //    .x(function(d){q= 0.95105*3000*(d[0]+66.4262);  return q;})
    //    .y(function(d){q= 3000*(-1*d[1]+ 18.2336);  return q;})
    //    .interpolate("linear");
...
dc.bubbleOverlay = function(root, chartGroup) {
    ...
    **/
    ...
    var _chart = dc.marginable(dc.multiSelectionChart(dc.colorChart(dc.baseChart({}))));
    ...
    _chart.isSelectedRow = function (d) {
        return _chart.isFilteredBy(_chart.keyAccessor()(d));
    };
...
};dc.menuChart = function(parent, chartGroup) {

    var _g;

    var _labelOffsetX = 7;

    var _labelOffsetY = 14;
    ...
    _chart.doRender = function() {
        _xScale = d3.scale.linear().domain([0, 1]).range([0, _chart.effectiveWidth()]);
